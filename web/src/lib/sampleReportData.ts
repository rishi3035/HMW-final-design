...
export const sampleReport = {
  ...
  consolidatedPatches: {
    nextjs: `// next.config.mjs (Next.js 14/15) - Consolidated Security Headers Patch
    ...
    return [
      {
        ...
        headers: [
          ...
          { key: 'Permissions-Policy', value: 'camera=(), microphone=(), geolocation=(), interest-cohort=()' },
          {
            ...
            value: "default-src 'self'; script-src 'self' 'unsafe-inline' https:; style-src 'self' 'unsafe-inline' https:; img-src 'self' data: https:; font-src 'self' data: https:; connect-src 'self' https:; frame-ancestors 'self';"
          }
        ],
      },
    ];
...
export default nextConfig;`,
    nginx: `# /etc/nginx/conf.d/security_headers.conf - Consolidated Nginx Headers Patch
# Resolves: Missing HSTS, Clickjacking, MIME Sniffing, Permissive CSP, and Referrer Leakage
...
server_tokens off;`,
    fastapi: `# FastAPI / Starlette Security Headers Middleware Patch
  ...
  findings: [
    {
      ...
      severity: "medium",
      confidence: "High",
      toolSource: "Deterministic Security Policy Check",
